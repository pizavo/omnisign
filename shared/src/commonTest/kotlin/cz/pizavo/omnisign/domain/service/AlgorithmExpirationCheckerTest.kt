...
class AlgorithmExpirationCheckerTest : FunSpec({
	...
	test("non-expiring algorithms are always VALID") {
		listOf(
			...
			AlgorithmExpirationChecker.check(alg, defaultConfig, today) shouldBe AlgorithmStatus.VALID
		}
	}
	
	test("algorithm is VALID on exactly its expiration date") {
		AlgorithmExpirationChecker.check(
	...
	test("RIPEMD160 expired before updateDate uses expirationLevel FAIL") {
		AlgorithmExpirationChecker.check(HashAlgorithm.RIPEMD160, defaultConfig, today) shouldBe
			AlgorithmStatus.EXPIRED_FAIL
	}
	
	test("null config falls back to DEFAULT and produces EXPIRED_FAIL for RIPEMD160") {
		AlgorithmExpirationChecker.check(HashAlgorithm.RIPEMD160, nullConfig, today) shouldBe
			AlgorithmStatus.EXPIRED_FAIL
	}
	...
	test("WHIRLPOOL expired before updateDate uses expirationLevel FAIL") {
		AlgorithmExpirationChecker.check(HashAlgorithm.WHIRLPOOL, defaultConfig, today) shouldBe
			AlgorithmStatus.EXPIRED_FAIL
	}
	
	test("RIPEMD160 expired before updateDate uses expirationLevel WARN when set to WARN") {
		AlgorithmExpirationChecker.check(HashAlgorithm.RIPEMD160, allWarnConfig, today) shouldBe
			AlgorithmStatus.EXPIRED_WARN
	}
	
	test("RIPEMD160 expired before updateDate uses expirationLevel IGNORE as WARN") {
		AlgorithmExpirationChecker.check(HashAlgorithm.RIPEMD160, allIgnoreConfig, today) shouldBe
			AlgorithmStatus.EXPIRED_WARN
	}
	...
	test("algorithm expired after updateDate uses expirationLevelAfterUpdate WARN") {
		...
		AlgorithmExpirationChecker.check(HashAlgorithm.RIPEMD160, config, today) shouldBe
			AlgorithmStatus.EXPIRED_WARN
	}
	...
	test("override date in future makes bundled-expired algorithm VALID") {
		...
		AlgorithmExpirationChecker.check(HashAlgorithm.RIPEMD160, config, today) shouldBe AlgorithmStatus.VALID
	}
	...
	test("override on non-expiring algorithm with past date before updateDate yields EXPIRED_FAIL") {
		...
		AlgorithmExpirationChecker.check(HashAlgorithm.SHA256, config, today) shouldBe
			AlgorithmStatus.EXPIRED_FAIL
	}
	...
	test("override on non-expiring algorithm with future date keeps VALID") {
		...
		AlgorithmExpirationChecker.check(HashAlgorithm.SHA256, config, today) shouldBe AlgorithmStatus.VALID
	}
	...
	test("effectiveExpirationDate returns override when present") {
		...
		AlgorithmExpirationChecker.effectiveExpirationDate(HashAlgorithm.RIPEMD160, config) shouldBe
			LocalDate(2040, 6, 15)
	}
	
	test("effectiveExpirationDate falls back to bundled date when no override") {
		AlgorithmExpirationChecker.effectiveExpirationDate(HashAlgorithm.RIPEMD160, defaultConfig) shouldBe
			HashAlgorithm.RIPEMD160.expirationDate
	}
	
	test("effectiveExpirationDate returns null for non-expiring algo with no override") {
		AlgorithmExpirationChecker.effectiveExpirationDate(HashAlgorithm.SHA256, defaultConfig).shouldBeNull()
	}
	...
	test("warningMessage uses override date when present") {
		...
		AlgorithmExpirationChecker.warningMessage(HashAlgorithm.RIPEMD160, config) shouldContain "2040"
	}
})

...
- `commonMain` — platform-agnostic domain: models, use cases, repository/port interfaces, error types, `platform/PasswordCallback.kt`, `Constants.kt`. **No DSS imports here.**
  ...
  - `domain/model/text/` — `LocalizableText` (`Keyed(MessageKey, args)` | `Literal`), the `MessageKey` enum (104 stable, `@Serializable` identifiers), and `EnglishMessages` (the bundled English templates behind `english()`). Frontends that localize resolve the key; the CLI, server, and logs fall back to `english()`.
  - `api/model/responses/` — the **server↔web wire contract** (`CapabilitiesResponse`, `SigningResultMeta`, `ResolvedConfigResponse`, …). It lives here, not in `server`, precisely so the Wasm client can deserialize what the Ktor routes serialize.
- `wasmJsMain` — HTTP-backed implementations of the *same* repository ports the JVM satisfies with DSS: `data/remote/Remote*Repository` (`Signing`, `Validation`, `Archiving`, `Config`, `TrustStore`, `ConfigArchive`, `Capabilities`) plus `BrowserProfileSelectionStore`, wired in `di/WebDataModule.kt` over one Ktor `HttpClient` (`expectSuccess = true`). Operations a browser cannot perform (`unlockToken`, `loadCertificatesFromFile`, signing with a local keystore) return a `Left` rather than throwing, so the ports stay substitutable.
...
- `config/` — all server-side configuration data classes:
  ...
  - Secrets: `SecretResolver` + `EnvVarSubstitution` expand `${NAME}` placeholders from environment variables; `SensitiveStringJacksonDeserializer` binds them into `Sensitive<String>`; `RemovedSecretFieldHandler` rejects retired secret fields.
...
- `auth/` — authentication domain:
  ...
  - `IdTokenVerifier` / `IdTokenVerificationException` — cryptographically verifies the IdP's `id_token` (JWKS via `jwks-rsa`) instead of trusting the UserInfo response, making the IdP's signing key the root of trust for identity assertions.
...
- `api/routes/` — one file per route group, all mounted under `/api/v1`:
  - `AuthRoutes` — `/auth/**`: `GET /login` (provider list), `GET /redirect/{name}` (OIDC initiation), `GET /callback/{name}` (OIDC exchange), `GET /session`, `POST /refresh`, `POST /logout`.
...
- **Wire DTOs live in `shared`, not in `server`.** The response classes the web client also has to deserialize are in `shared/commonMain/.../api/model/responses/`: `CapabilitiesResponse`, `SigningResultMeta`, `TimestampResultMeta`, `GlobalConfigResponse`, `ProfileConfigResponse`, `ResolvedConfigResponse`, `TimestampServerSummary`, `TrustedCertificateResponse`. That shared package *is* the server↔web contract — change it and both sides move together.
- `server`'s own `api/model/responses/` holds only server-side DTOs: `ApiError`, `HealthResponse`, `LoginOptionsResponse`, `SessionResponse`, `TokenResponse`. `api/model/requests/` holds `RefreshTokenRequest`.
...
- `api/RouteUtils.kt` — shared route helpers (`rateLimitedIf`, `requireOperation`). `api/RequestLanguage.kt` reads the caller's `Accept-Language` and feeds `ValidationParameters.language`, so DSS validation reports are localized **per request**.
...
- `di/ServerModule.kt` — server Koin module; provides `ServerConfig`, `ServerConfigLoader`, `PasswordCallback` (always null), `CoroutineContext` (IO), `HttpClient` (CIO, JSON content-negotiation), `JwtSessionService`, `OidcDiscoveryService`, `OidcUserInfoService`, `IdTokenVerifier`, `PkceService`, the two Exposed stores, and a lazy `Database` singleton (`Database.connect("jdbc:sqlite:…")` rooted at `sessionsDbFile`).
...
- **Server authentication flow**: `configureAuthentication()` registers a `jwt-api` Bearer provider (always installed, even when auth is disabled) and one `oidc-{name}` OAuth2 provider per `OidcProviderConfig`. `configureRouting()` wraps all operational routes in `authenticate(jwt-api)` when `auth.enabled = true`; health and all `/auth/**` routes remain publicly accessible. The redirect step calls `PkceService.begin()` (S256 challenge, verifier stored by `state`); the callback calls `PkceService.consume()` for the token exchange and `IdTokenVerifier` on the returned `id_token`. Post-login checks (`isEmailDomainAllowed`, `areRequiredClaimsSatisfied`) then run before a JWT access token and an opaque, rotating refresh token are issued.

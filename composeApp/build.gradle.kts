 ...
 * version to common Compose code, so both the JVM desktop and the Wasm web
 * target can render it (e.g. in the Help panel) without a JVM-only manifest
 * lookup. Mirrors the CLI module's `generateVersionProperties` precedent.
 */
val generateBuildConfig by tasks.registering {
	group = "build"
	description = "Generates BuildConfig.kt containing the project version."
	...
	doLast {
		...
		file.writeText(
			buildString {
				appendLine("package cz.pizavo.omnisign")
				appendLine()
				appendLine("/**")
				appendLine(" * Build-time constants generated by the Gradle `generateBuildConfig` task.")
				appendLine(" *")
				appendLine(" * Do not edit by hand; regenerated on every build from the project version.")
				appendLine(" */")
				appendLine("internal object BuildConfig {")
				appendLine("    /** Application version, sourced from the Gradle `releaseVersion` project property. */")

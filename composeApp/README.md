...
```shell
# JVM-target tests (ViewModels, platform implementations)
...
│           └── viewmodel/           MVVM ViewModels (PdfViewerViewModel, SignatureViewModel, ProfileViewModel)
│
├── jvmMain/             Desktop-specific implementations
│   └── kotlin/cz/pizavo/omnisign/
│       ├── main.kt                  JVM entry point — Koin bootstrap, JBR decorated window
│       └── ui/platform/             actual implementations (PDFBox renderer, JBR title bar, AWT cursors)
│
...
- **Island layout** — The desktop shell is inspired by the IntelliJ "Island" UI: a seamless toolbar
  ...
  and width are managed with local `remember` state.
...
- **MVVM** — ViewModels (`PdfViewerViewModel`, `SignatureViewModel`, `ProfileViewModel`) expose
  `StateFlow`-based state consumed by composables via `collectAsState()`. Dependencies are
  resolved through Koin.
...
- **Lumo design system** — A custom theme layer (`lumo/`) supplies colors, typography, and a
  component library (buttons, cards, tooltips, text fields, …) used across all composables.

 ...
 *
 * @property text The message body shown in the snackbar, as a locale-agnostic [ToastText] that
 *   [cz.pizavo.omnisign.ui.toast.ToastHost] resolves at render time.  Keep short — Compose
 *   snackbars constrain content width and wrap awkwardly past ~2 lines.
 * @property actionLabel Optional label for an action button rendered on the right of the
 *   snackbar, as a [ToastText].  `null` (default) suppresses the button entirely.  When non-null
 *   pair it with [onAction]; clicks invoke [onAction] then clear the toast for every live host.
...
data class ToastMessage(
	val text: ToastText,
	val actionLabel: ToastText? = null,

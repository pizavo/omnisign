...
Third-party material copied into this repository is credited in [NOTICE.md](NOTICE.md). Every
library the installers ship is listed with its licence and copyright in
[THIRD-PARTY.md](THIRD-PARTY.md), and the full text of each licence is in [`licenses/`](licenses/).
Both are regenerated by `./gradlew generateThirdPartyNotices` and verified in CI.

OmniSign is built on [EU DSS](https://github.com/esig/dss), used under the GNU LGPL v2.1 or later.

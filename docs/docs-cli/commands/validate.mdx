...
| `-d, --detailed`           | Expand every certificate in a chain into its full parsed dump (all fields and extensions), and append a `── Configuration ──` block showing the resolved policy, revocation, EU LOTL and trusted-list settings |
...
── Trusted List Warnings ──
  ⚠ Could not refresh 1 trusted list (eidas.gov.ie). Qualification assessment for
...
- **Errors** — Constraint checks that failed according to the active validation policy.
  ...
  The overall `VALID` result remains the authoritative verdict.
...
a network error), a `── Trusted List Warnings ──` section is printed at the **end** of the

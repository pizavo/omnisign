...
See [Algorithms](../algorithms.mdx) for the full list of supported algorithms.

| Setting                                   | Description                                                                                                                                        | Default |
|-------------------------------------------|----------------------------------------------------------------------------------------------------------------------------------------------------|---------|
| Hash algorithm                            | Digest algorithm for the signature (see [full list](../algorithms.mdx#hash-algorithms))                                                            | SHA-256 |
| Encryption algorithm                      | Signing key algorithm (see [full list](../algorithms.mdx#encryption-algorithms); auto-detected from certificate)                                   | Auto    |
...
- **Alert if not on EU LOTL** — when on, the validation panel flags every signature whose trust
  ...
  [Validating Signatures → EU LOTL membership](validation.mdx#eu-lotl-membership).
- **Trusted list refresh interval (hours)** — how often the EU LOTL and custom lists are
  refreshed automatically in the background (minimum 1 hour, 4 by default). Keep this at or
  below **6**: the ETSI validation policy considers a trusted list fresh only for 6 hours after
  its last successful synchronization, so a longer interval leaves qualified validations
  reporting *"the trusted list is not considered as fresh"* for the rest of each cycle.
...
[profile editor](profiles.mdx#trusted-certificates).
...
scope is rejected. The read-only [Trusted Certificates panel](trusted-certificates.mdx) shows the
...
A **Build Custom TL** button opens the [Trusted List builder](tl-builder.mdx) dialog for creating new
...
| Glob patterns | File path patterns to watch (e.g. `/data/signed/**/*.pdf`). A pattern is rejected if it is not an absolute path, if it matches no files, or if the platform cannot parse it — an unclosed `[` or `{`. |
...
Below the installation indicator the section reports what the last run actually did — when renewal
last succeeded, and the last run's outcome with its checked / renewed / error counts. Warning rows
appear when documents have passed their preservation deadline, when runs have failed since the last
success, and when a run began and never finished.

That last case is the one nothing else can report. Renewal runs in a separate process, and a process
killed by a restart or a shutdown writes no run record and fires no notification — so the run that
follows it is the first thing able to notice. OmniSign distinguishes a run that is *still executing*
from one that was killed, and only calls the second interrupted.

:::tip[You do not have to open Settings to find out]
When renewal needs attention — any failure since the last success, or a run that never finished — the
**settings button in the toolbar carries a badge**. The status is refreshed when the app starts and
whenever the settings dialog is opened or closed. It is not polled while the window sits open, so a
run that fails overnight shows up the next time you come back to the app.
:::

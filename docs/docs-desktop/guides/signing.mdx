...
A document can also be opened from outside the app, through the system's **Open with** menu — see
[Opening documents](../features.mdx#opening-documents).
...
<ZoomableImage src={useBaseUrl('/img/desktop/signing-discovery.avif')} alt="Opening the signing dialog and certificate discovery" />
...
[Troubleshooting → PKCS#11 tokens not detected](../troubleshooting.mdx#pkcs11-tokens-not-detected)
...
{/* Animated AVIF (`avis`): Docusaurus's markdown image pipeline cannot read its dimensions, so it stays raw JSX and reaches the enlarge overlay via ZoomableImage. */}
<ZoomableImage src={useBaseUrl('/img/desktop/signing-rescan.avif')} alt="Rescanning for tokens" />
...
in that case the [timestamping dialog](timestamping.mdx) can later add the revocation data to
...
grayed out and cannot be selected. See [Algorithms](../algorithms.mdx) for the full list
...
- **Continue anyway** — proceed to the **Save As** dialog and keep the output despite missing revocation
...
If the output file is already covered by an existing job (detected from its glob patterns), no dialog
is shown at all — the file is renewed by that job regardless.

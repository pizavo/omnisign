...
failures depending on the [algorithm constraint settings](guides/settings.mdx#algorithm-constraints).
...
:::note[RIPEMD-160 key pairings]
RIPEMD-160 signs with RSA (PKCS#1 v1.5), ECDSA, and Plain-ECDSA, but **not with RSA-PSS or
DSA** — DSS 6.3 defines no `SignatureAlgorithm` pairing RIPEMD-160 with those. Choose plain
RSA or ECDSA when signing with RIPEMD-160.
:::
